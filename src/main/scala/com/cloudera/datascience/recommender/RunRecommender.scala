...
object RunRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sparkContext.setCheckpointDir("hdfs://node-master:9000/tmp")
    // Cambiado a local
    //spark.sparkContext.setCheckpointDir("file:///tmp/")

    val base = "hdfs://node-master:9000/user/hadoop/data/profiledata_06-May-2005/"
    ...
    runRecommender.model(rawUserArtistData, rawArtistData, rawArtistAlias, args(0).toInt)
    runRecommender.evaluate(rawUserArtistData, rawArtistAlias)
    runRecommender.recommend(rawUserArtistData, rawArtistData, rawArtistAlias, args(0).toInt)
  }
...
class RunRecommender(private val spark: SparkSession) {
  ...
  def evaluate(
    ...
    println("Esto es lo más escuchado (mostListenedAUC):")
    ...
    println("(evaluations.sorted.reverse.foreach):")
